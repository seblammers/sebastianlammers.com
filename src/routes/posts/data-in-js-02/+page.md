...
date: '2023-04-25'
draft: false
image: '/preview-post_data-in-js-02.png'
categories:
  - 'JavaScript'
  - 'data'
  - 'basics'
...
let url =
	'https://raw.githubusercontent.com/seblammers/sebastianlammers.com/master/src/routes/api/data.json/penguins.json';
...
1. You want to get _some kind subset_ to peek inside your data without being overwhelmed by rendering _all of it_.
2. You want to get a _specific subset_ to e.g. visualize/work on separately from the rest.
...
We take a look at the _head_ of the body of data...
...
Secondly, the number specifying the end of our slice is _not_ included!
...
But sometimes you might be interested in a _particular subset_ of your data.
...
Let's say you are interested in what penguins Rita hangs out with on their home island called _Dream Island_.
To find out more about _Dream Island_, let's get all observations where `island === "Dream"`.
...
Now you can work with the data that is specifically about the observations from _Dream Island_.

I'm not rendering these to the screen, but you can see that 124 out of the 344 observations are about penguins from _Dream Island_.
...
Now, isn't _that_ convenient? All we need now is a way to go through all entries in our **array** and inside each entry, visit our variable of interest.
...
We take our **array** called _data_ and use `map()` to visit each entry in it.
Each entry is an **object** that we can nickname _row_ to refer to its relation in the spreadsheet-style image.
Inside each of the rows, we visit the _species_-variable via dot notation.
